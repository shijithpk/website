...
description = "An analysis of how well apps like Spotify cover critically-acclaimed albums"
[extra]
...
This analysis will allow us to see what holes there are in the libraries of these music services, and also to quantify in concrete terms the difference in coverage between them. Listeners can hopefully use this post to guide them when it's time to pick a subscription.


### How I went about it

First off, I chose 10 genres (Pop, Rock, Jazz etc.) to represent the variety in western music. Could've gone for more, I've excluded country and folk music for example, but I think the 10 genres selected cover most of what the West has to offer. 

Then I researched which books, magazines or websites are respected in each genre, and what albums they have adjudged to be the best of all time (the 'canon') or the best of recent times. For example, for EDM (electronic dance music), the canon was taken from a list of the [most influential albums](https://mixmag.net/feature/the-50-most-influential-dance-music-albums-of-all-time) by dance music publication Mixmag. 
...
Some genre afficionados could question why one list was chosen and not another. Going for a specific list is actually down to mundane factors like availability online or how it's been compiled. 

For example, i've used the magazine [Downbeat](https://www.downbeat.com/) to get lists of the best contemporary blues albums for each year. By some accounts, [Living Blues](https://livingblues.com/) magazine is the more respected blues publication, but their annual list isn't compiled by critics but based on radio airplay, so I decided not to use it.

Also, I accept that these lists are subjective. Ideally, a streaming service should be able to accommodate any best albums list that critics come up with. But taking any one list should good enough for a starting point.

Some might also question the use of albums, instead of individual songs, to compare streaming services. That people [don't really listen to whole albums](https://www.digitalmusicnews.com/2019/10/04/are-albums-dying/) anymore, that they don't consume their music that way. But I felt that checking for 100 whole albums, for example, would be a sterner test for these services than checking for 100 individual songs, so I went with albums.
...
I guessed wrong though, because outside of Spotify and Apple, there are still significant gaps when it comes to music coverage. If you primarily listen to western music and want to subscribe to a streaming service, it's hard to recommend anything other than Spotify or Apple.
...
After hip-hop, EDM I guess has had the most influence on the sound of today's pop music. But for such an influential genre, it doesn't really get much coverage, with only two services Spotify and Apple getting atleast a 9.
...
Some past acts whose albums are missing are Boyz II Men ('90s album II missing from YouTube Music) and Alicia Keys (2016 album 'Here' not on Amazon Music).
...
Some of the tracks from that album will be there in other compilations available on each service. So you could technically recreate the experience of listening to the album by making a playlist that cuts and pastes those tracks together. That may feel like you're listening to the album, but it'll still have to be marked as unavailable.

I have exercised my discretion at other times though. If there's a compilation by another name that has all the tracks of the album and there's not too many extra pieces to sit through, I've accepted that as a substitute.
...
Surprisingly, the major services — Spotify, Apple, YouTube, Amazon — all do better in covering Blues than Jazz. You'd think it would be the opposite given how Blues and Blues artists aren't as well-known. Spotify even manages to score a 9 here.
...
The Gramophone guide recommends the 1961 version, so even if the 1962 version is available on a service, it doesn't count. We may not be privy to the logic why the guide went for one over the other, but we assume that there are good reasons, and try not to deviate from its choices.
...
I guess one could question if a category called 'World Music' should be here. A diverse collection of genres from around the globe, this isn't actually music from the West. But to the extent 'World Music' is a beneficiary of western musical tastes, it felt important to include. (In fact, a more honest title for this blog post is not "What's the best streaming service in India for western music?", but "Which streaming service in India best caters to western musical taste?")
...
) is the premier list of 'best albums'. It's been around since 2003, with the latest version dating to 2020, and [hundreds](https://www.rollingstone.com/music/music-news/voters-500-greatest-albums-list-1062225/) of critics are involved in compiling it. I wanted to see how the various services did when it came to covering the latest RS 500. 5 of the services did well with over 400 of the 500 albums on the list.
...
In a way, what we're actually looking at in this post is the extent of these legal relationships with global music groups, and the 'mapping' from that to different genres and how they're covered. The more music groups a streaming service has deals with, and the more extensive the nature of those deals, the better a service's coverage is across genres.
...
**Gaana** — Pretty much all albums associated with Universal Music are missing from the service, which probably means Gaana doesn't have a deal with them. Albums from The Beatles, Nirvana and Kanye West are missing as a result. Now when I started my research a few months back, I think these albums were there then, and them not being there now means the deal has lapsed and not been renewed. Even if I'm mistaken about that, the fact remains these albums aren't there now, and this absence affects Gaana in all genres. Its overall rating would jump from a lowly 3.0 to 5.3 if it had a deal with Universal.
...
Just felt I should acknowledge how streaming services aren't the only options available to users when it comes to getting music. And that the most popular option isn't here only because it's technically a video-sharing platform.
...
False negatives are albums that are there in a streaming service's library, but my code couldn't find them. I'm trusting my code is good enough that it gets most of the albums that are available, and even if I do miss a few of them, the service's rating won't be affected by much in most cases. One way to avoid false negatives is to search for albums manually, but I felt that would defeat one of the goals here which is to do everything programatically as much as possible.
...
Of course, there is the danger that a critic could choose an album that fans don't listen to much. So if it's in the long tail in terms of popularity, should a service be penalised for not having it? Those albums being unavailable might not affect how a listener enjoys a service as long as it provides everything on the popular end of the spectrum.
...
**Categories too broad?** — Another issue that could affect the utility of this analysis to listeners is that the categorisation might be too broad for some. For example, I've used metal as a genre, but listener tastes might be so advanced that they listen to sub-genres like death metal or black metal or shoegaze metal, and may not necessarily identify with the larger metal genre. In fact, it might even be important for them to identify *against* such broad tastes. 
...
I've focused on western music here, but someone else could take this work forward by looking at the various music services and looking at how well they cover Indian music. The code's all there in my [Github repo](https://github.com/shijithpk/music-streaming-india), all you would need is a dataset of albums for each Indian language from the past and present that are critically acclaimed. 
