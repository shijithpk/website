...
Aggregating figures at a state level, 55% of Kerala's panchayats have grown hotter by over half a degree celsius in the last decade or so.
...
Have chosen the panchayat as the unit of study, because it represents the lowest level of government, and hopefully the findings here will spur village authorities into action by taking steps to bring temperatures down and improve livability. 
...
This is just speculation though. If this story gets picked up by a media publication, I'll look into it further and ask some academic or expert to provide more insight.
...
The panchayat that has heated up most is Vijayapuram in Kottayam, which increased by 1.7°C (from 32.3°C in 2008-12 to 34°C in 2018-22). So what's led to this rise in surface temperature? 
...
Development in circled area on the right—and increased surface temperature—may be a response to the presence of Manarcadu junction, the hub of neighbouring panchayat Manarcadu, being inside Vijayapuram borders. In fact, Manarcadu's own panchayat office is at the junction too.
...
### Changes from a district point of view
...
![Kerala's districts ordered by the % of panchayats whose avg. surface temp. has risen by 0.5 plus degrees](IMAGES/04_districts_most_panchayats_heated_up.png)
...
## Side notes
Caveats, doubts and other random thoughts I couldn't fit into the copy above. (I'm done with my story, so you can stop reading if you want, just doing some housekeeping from this point on.)
...
But we can't do that with readings from weather stations, because of how few weather stations there are in Kerala compared to panchayats. There are 941 panchayats and [around 100](http://aws.imd.gov.in:8091/state.php?id=KERALA) working weather stations, so if you're looking to get panchayat-wise temperatures, you're out of luck. 
...
ERA5-Land being a reanalysis product would've still been acceptable for our purposes, but for the poor resolution of the dataset, around 9 km. So a single temperature from that dataset will be for a 9 x 9 or 81 sq km area. But 81 sq km could comprise multiple panchayats as a majority of Kerala's panchayats are less than 40 sq km in area. 

So chances are you'll have to use the same figure for multiple panchayats and you won't get any differentiation between them.
...
Plus the heating up of rural Kerala isn't that evident to people during the monsoon months. It's during the 'fair weather' months of December to May that these hellish temperatures come into play. So even if more data is available for these less-cloudier months, that is fine as we continue getting an insight into the discomfort people feel then.
...
Another thing I've worked hard at doing is to make the results in this story completely reproducible. The code is all there in a [jupyter notebook](https://github.com/shijithpk/hottest-panchayats-kerala/blob/main/code_final.ipynb) on Github, and it can be run cell by cell to get the original results. Whenever I've worked with geospatial data in the past, it's usually been in GUI tools like QGIS, and it's hard for others to replicate its results. There are no such issues here with the workflow fully in code.
...
But I was reminded of it by this [2022 story](https://peabee.substack.com/p/17-one-kitchen-hundreds-of-internet) by techie Prashant Baid that hyper-focused on one Bangalore cloud kitchen and exposed its shady practices with the help of data. I'm going to make a conscious effort to do similar investigative work that isn't about making grand claims.
